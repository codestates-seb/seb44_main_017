...
public interface AdminRepository extends JpaRepository<Admin, Long> {
    ...
    @Query(value = "select P.product_id,P.member_id,P.category,P.name,P.title,P.content,P.image_link,P.modify_at,P.create_at,P.price,P.productlike,P.view,P.condition_value from admin a join product P ON a.admin_id = P.admin_id where P.issell=:issell and a.admin_id=:ID order by P.view desc", nativeQuery = true)
    Page<queryget.product> findAdminProductView(@Param(value = "ID") Long ID, @Param(value= "issell") boolean issell, Pageable pageable);

    @Query(value = "select P.product_id,P.member_id,P.category,P.name,P.title,P.content,P.image_link,P.modify_at,P.create_at,P.price,P.productlike,P.view,P.condition_value from admin a join product P ON a.admin_id = P.admin_id where P.issell=:issell and a.admin_id=:ID order by P.productlike desc", nativeQuery = true)
